...
The client is a fully installable PWA (Android, Windows, macOS, Linux via
Chrome/Edge, and iOS/iPadOS via Safari's Add to Home Screen). This layer is
entirely additive — no existing route, component, business logic, or auth
flow was changed to add it.

- **Manifest + icons**: `client/vite.config.js` (the `VitePWA(...)` block)
  defines the manifest — name, standalone display, theme colors, and a
  full icon set including maskable variants for Android's adaptive icons.
  Source icons live in `client/public/icons/`; the brand mark (a wound
  yarn ball) was custom-drawn to match `tokens.css`, not a stock icon.
- **Service worker & offline**: generated by `vite-plugin-pwa` (Workbox)
  at build time — see `dist/sw.js` after building. Static assets and the
  app shell are precached for offline load; `GET /api/**` requests use a
  **NetworkFirst** strategy (always prefers a live network response, only
  falls back to the last-seen response when offline). Every write
  (`POST`/`PUT`/`PATCH`/`DELETE`) is untouched by the service worker and
  always goes straight to the network — nothing about how stock entries,
  beam production, or any mutation is saved has changed.
- **Updates**: `registerType: 'prompt'` — a new deployed version installs
  in the background and waits. `client/src/pwa/PwaUpdateNotifier.jsx`
  polls for new versions every 30 minutes and, when one's ready, shows a
  dismissible "Update available" toast (via the existing `ToastContext`)
  with a **Reload** button. Nothing force-reloads a page while someone
  might be mid-form.
- **Install prompt**: `client/src/pwa/InstallPrompt.jsx` shows a small
  branded "Install Yarn ERP" banner using the browser's native
  `beforeinstallprompt` event on Chromium/Edge/Android, and a "tap Share →
  Add to Home Screen" hint on iOS Safari (which has no such event).
  Dismissing it snoozes for 7 days (stored in `localStorage`, client-side
  only).
- **Offline indicator**: `client/src/pwa/OfflineBanner.jsx` shows a slim
  top banner whenever the browser goes offline, and hides itself the
  moment connectivity returns.
- **iOS splash screens**: `client/public/splash/` + the
  `apple-touch-startup-image` links in `index.html` cover the common
  iPhone/iPad screen sizes so the app doesn't flash a blank white screen
  on launch from the home screen.

### Verifying it locally

The service worker is intentionally **disabled in `npm run dev`**
(`devOptions.enabled: false` in `vite.config.js`) so it can't add caching
confusion on top of the dev proxy. To test the real PWA build:
...
Then, in Chrome DevTools → Application tab: check *Manifest* (icons,
colors), *Service Workers* (registered, `sw.js`), and run *Lighthouse* →
PWA + Performance audit. To see the install prompt, offline banner, and
update toast in action, install the app from the browser's install icon,
then toggle the Network tab to "Offline" and reload.

If you deploy the server and client together behind one origin, no config
changes are needed. If they're on different origins, make sure `VITE_API_URL`
points at the API's absolute URL — the API caching rule matches same-origin
`/api/**` requests, so cross-origin API calls will simply bypass the
service worker's caching (they'll still work, just without the offline
fallback for GETs).
